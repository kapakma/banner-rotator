...
class Rotator {
  constructor(el, opts) {
    ...
    this._cssTransition =
      SUPPORT.transition && !ANDROID2 ? this._options.cssTransition : false;
  ...
  initItems() {
    ...
    this._$items.each(
      $.proxy(function (n, el) {
        ...
        $img
          .addClass('br-img')
          .data('src', $img.data('src') || $img.attr('src'));
        ...
        if (this._options.tooltip === 'image') {
          ...
          $item.data({
            tooltip: $item.find('>.tooltip').html() || $img.attr('title'),
          });
        }
        ...
        $item.data({
          easing: getEasing($item.data('easing'), this._cssTransition),
  ...
  initCPanel() {
    ...
    if (this._options.groupButtons && this._options.thumbnails !== 'bullet') {
      ...
      this._options[`thumb${size}`] = this._options[`button${size}`] = Math.max(
    ...
    if ($el.length) {
      ...
      if (!isNone(this._options.thumbnails)) {
        ...
        if (this._options.responsive) {
          ...
          this._$cpanel[outerDim](true) > this[`_stage${capitalize(dim)}`]
        ) {
          this.resizeCPanel();
          this.addThumbScroll();
        }
  ...
  initNavThumbs() {
    if (
      ...
      $.inArray(this._options.navButtons, ['large', 'outside', 'outer']) > -1
    ) {
      let $prevNav,
        $nextNav,
        $prevWrapper = $('<div/>', {
      ...
      $wrappers
        .each(
          $.proxy(function (n, el) {
            const $thumb = $(el).find('>.br-nav-thumb').css({
      ...
      if (this._options.navButtons === 'large') {
        ...
        $wrappers
          ...
          .on(`mouseleave${this._namespace}`, function () {
            $(this).removeClass('br-hover-on').addClass('br-shrink');
          });

        $prevNav
          .add($nextNav)
          .on(`mouseenter${this._namespace}`, $.proxy(this.showNavThumb, this))
      ...
      for (let i = 0; i < this._numItems; i++) {
        ...
        switch (this._options.thumbnails) {
          ...
          case 'image':
            ...
            $img
              .data({
                src: this._$items.eq(i).find('>img.br-img').data('thumb'),
              })
              .addTransitionClass('br-opacity-transition')
      ...
      $.proxy(function (n, el) {
        ...
        $.each(
          ...
          $.proxy(function (i, dir) {
            ...
            $.each(
              ...
              $.proxy(function (j, prop) {
                $el.data(
                  ...
                  getValue(
                    $el.data(prop),
                    this._options[`layer${capitalize(prop)}`]
                  )
                );
              }, this)
            );
            $el.data(
              `easing${dir}`,
              getEasing($el.data(`easing${dir}`), this._cssTransition)
            );
          }, this)
        );
      }, this)
              ...
              $img.brHandleImage($img.attr('src'), {
                complete: $.proxy(function () {
                  ...
                  if (CHROME) {
                    $thumb.data(
                      'tooltip-kb',
                      new KenBurns($content, $(el).data('kb-effect'))
                    );
                  }
                }, this),
              });
  ...
  displayTooltip($thumb) {
    ...
    if (this._orientation === 'vertical') {
      ...
      top =
        $thumb.offset().top - (this._$tooltip.height() - $thumb.height()) / 2;
    } else {
      top = $base.offset().top;
      ...
      left =
        $thumb.offset().left - (this._$tooltip.width() - $thumb.width()) / 2;
    }
    ...
    this._tooltipId = setTimeout(
      $.proxy(function () {
        this._$tooltip
          ...
          .show()
          .animate({ opacity: 1 }, Rotator.ANIMATE_SPEED, function () {
  ...
  selectSlide(e) {
    const index =
      typeof e === 'number' ? parseInt(e, 10) : $(e.currentTarget).index();
    if (
  ...
  resizeCPanel() {
    ...
    this._$thumbPanel
      .css(
        dim,
        Math.min(
          this._$screen[dim]() - this._cpanelMargin - this._buttonSize,
  ...
  touchStart(e) {
    ...
    if (e.originalEvent.touches.length === 1) {
      ...
      this._$rotator
        .on(`touchmove${this._namespace}`, $.proxy(this.touchMove, this))
